  ...
  const handleImageSubmit = (e) => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError("Image Uplaod Failed (2 mb max per image)");
          setUploading(false);
        });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.imageUrls.length < 1) {
        return setError("You must Upload atleast 1 image to proceed");
      }
      if (+formData.regularPrice < +formData.discountPrice) {
        return setError(
          "Discounted Price must be lower than the Regular Price1"
        );
      }
      setLoading(true);
      setError(false);
